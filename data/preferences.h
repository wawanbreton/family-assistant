...
public:
    void registerPreference(const PreferenceEntry::Enum entry, const int type, const QVariant& default_value);

    QString getString(const PreferenceEntry::Enum entry) const;

    int getInt(const PreferenceEntry::Enum entry) const;

    QList<int> getIntList(const PreferenceEntry::Enum entry) const;
